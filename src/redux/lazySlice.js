...
/**
 * Returns the Redux state for a slice, lazily initialized/filled-in by `lazyReducer`.
 *
 * NOTES:
 *
 * Multiple stores are not supported, as the lazy state resides in the slice object.
 *
 * This hook is incompatible with any `useSelector` depending on the slice state, as the slice state is not updated until a slice action occurs.
 * 
 ...
 * 
 * Also, if this hook is used multiple times, the `lazyReducer` functions must generate equivalent states independently of their invocation order.
